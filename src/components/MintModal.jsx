        ...
        <h3 className="form-header">Mint {props._metaData.name}</h3>
        {
          props.userAccount ? 
            <div class="flex flex-col font-mono text-white">
              <img className="my-4 mx-auto w-6/12 border-cyan"
                src="/static/images/NauticalMindsEP.jpg"/>
              <div className="w-6/12 mt-2 mb-4 mx-auto">
                <div className="flex">
                  <span className="w-4/12 text-right my-0.25 mx-0.5">Artist:</span>
                  <span className="w-8/12 text-left my-0.25 mx-0.5">{props._metaData.artist}</span>
                </div>
                <div className="flex">
                  <span className="w-4/12 text-right my-0.25 mx-0.5">Album:</span>
                  <span className="w-8/12 text-left my-0.25 mx-0.5">{props._metaData.album}</span>
                </div>
                <div className="flex">
                  <span className="w-4/12 text-right my-0.25 mx-0.5">Album Year:</span>
                  <span className="w-8/12 text-left my-0.25 mx-0.5">{props._metaData.albumYear}</span>
                </div>
              </div>
                  ...
                  props._metaData.trackList.map((tr)=>{
                    return (
                      <div className="flex m-auto">
                        ...
                        <span className="px-1 text-left">{tr.title} ({tr.fileType}, {bitrateToHumanReadable(tr.bitrate)})</span>
                        <span className="px-1 ml-auto mr-0 text-right">{secondsToMMSS(tr.size * 8 / tr.bitrate)}</span>
                      </div>
                    )
                  })
                }
              ...
              <div className="w-8/12 text-center my-2 m-auto italic">
                NMEP is an ERC 721 token on the public Ethereum blockchain,
                  that also gives you certain access on nauticalminds.com
                  the terms of such governed by Nautical Records LLC Terms of Service.
              </div>
